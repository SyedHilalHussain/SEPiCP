...
def clean_dataset(data):
    ...
    try:
        ...
        q3_col = next((c for c in ['q3', 'q3_semester', 'semester'] if c in df.columns), None)
        if q3_col and 'q3_4_text' in df.columns:
            df[q3_col] = df[q3_col].where(df[q3_col] != 'Other', df['q3_4_text'])
