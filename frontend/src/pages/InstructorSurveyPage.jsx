...
export default function InstructorSurveyPage() {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ background: '#1e3a8a', borderRadius: 20, padding: '32px 40px', marginBottom: 20 }}>
          <p style={{ color: '#93c5fd', fontSize: 12, fontWeight: 700,
            textTransform: 'uppercase', letterSpacing: 3, marginBottom: 12 }}>
            Course Code
          </p>
          <div style={{ fontSize: 52, fontWeight: 900, letterSpacing: 12,
            color: '#ffffff', fontFamily: 'monospace' }}>
            {courseCode}
          </div>
          <button onClick={handleCopy}
            style={{ marginTop: 20, padding: '10px 20px', borderRadius: 10,
  ...
  if (step === 'confirm') {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <button onClick={() => setStep('fill')} disabled={loading}
          style={{
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 20px 100px' }}>
      ...
      {/* Sections */}
      {INSTRUCTOR_SECTIONS.map((section, sIdx) => {
      ...
      <div style={{ position: 'sticky', bottom: 24, zIndex: 10 }}>
        <button onClick={handleSave} disabled={loading || !isFormReady}
          ...
          {loading ? 'Saving...' : (
            isFormReady
              ? <>Save &amp; Review <ChevronRight size={20} /></>
              : <>{filled}/{required.length} required fields filled — complete to continue</>
          )}
        </button>
      </div>
    </div>
